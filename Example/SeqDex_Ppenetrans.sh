...
RDP=~/../../media/bioinfio_group64b/DATA/database/rdp16s
...
SCRIPT=~/Documents/Bioinfo/Rscript/
#taxonomy information to be used. Can be NT of NTNR
TAX=NTNR
#machine larning algorithm to be used. Can be RF, SVM, BOTH
MLALG=BOTH
#target taxonomy name to be used to identify the target cluster
TRG=unclassified_Bacteroidetes
...
#find rRNAs in contigs
	barrnap -kingdom bac --threads "$THREADS" ../"${2}".fasta > barrnap16s_contigs.gff
...
if [ $TAX == "NT" ]; then
	...
	Rscript "$SCRIPT"/SeqDex/Taxonomy.R --blast ContigsvsNt.txt --taxaLevels 1,3 --aliLength 200 --Xid 70 --TaxonDensity 0.75 --diamond ContigsvsNr_mod.txt --aliLengthNr 70 --XidNr 80 --network ../Coverage/contigNet.txt --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2
fi
...
#--threads number of threads to be used 
	Rscript "$SCRIPT"/SeqDex/GCKmersCov.R --contigs "${2}".fasta --Kmers 3 --covSingle Coverage/onlymapping_sorted_SINGLE.bed --covPaired Coverage/onlymapping_sorted_PAIRED.bed --threads "$THREADS"
fi
cd ..
...
#--verbose set to true to print on the terminal performance statistics and percentage of error committed by RF model 
	Rscript "$SCRIPT"/SeqDex/RFT.R --gcCovKmersTable ../Coverage/gckCovTable.txt --rRNA16S ../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../Taxonomy/1taxonomyIteration.txt --network ../Coverage/contigNet.txt --minContigLen 1000 --targetName Bacteria --TaxaName superkingdom --replace T --threads "$THREADS"  --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2 --verbose T
	cd ..
	if [ $CLUSTER == "yes" ]; then
		echo "umap e dbscan"
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
#--verbose set to true to print on the terminal performance statistics and percentage of error committed by SVM model 
	Rscript "$SCRIPT"/SeqDex/SVM.R --gcCovKmersTable ../Coverage/gckCovTable.txt --rRNA16S ../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../Taxonomy/1taxonomyIteration.txt,../Taxonomy/2taxonomyIteration.txt --network ../Coverage/contigNet.txt --cross 5 --cost 1e-1,1e3 --gamma 1e-5,1e-1 --scale F --minContigLen 1000 --targetName Bacteria,Cytophagia --TaxaName superkingdom,class --threads "$THREADS" --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2 --verbose T
	cd ..
	if [ $CLUSTER == "yes" ]; then
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
		...
		Rscript "$SCRIPT"/SeqDex/Clustering.R --modelOutput ../SVMoutput/2outputSVM.txt --gcCovKmersTable ../Coverage/gckCovTable.txt --rRNA16S ../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../Taxonomy/2taxonomyIteration.txt  --network ../Coverage/contigNet.txt --input Kmers --minContigLen 1000 --targetName "$TRG" --threads "$THREADS" --ncomponents 2 --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
#--verbose set to true to print on the terminal performance statistics and percentage of error committed by RF model 
	Rscript "$SCRIPT"/SeqDex/RF.R --gcCovKmersTable ../Coverage/gckCovTable.txt --rRNA16S ../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../Taxonomy/1taxonomyIteration.txt,../Taxonomy/2taxonomyIteration.txt --network ../Coverage/contigNet.txt  --minContigLen 1000 --targetName Bacteria,Cytophagia --TaxaName superkingdom,class --replace T --threads "$THREADS" --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2 --verbose T
	cd ..
	if [ $CLUSTER == "yes" ]; then
...
#--taxonomy path and filenameto output of Taxonomy.R
#--network network file 
#--inputvariables to be used in clustering algorithm
#--minContigLeng minimum length of contigs to be considerend by RF
#--targetName name of the target taxonomy of interest
#--threads number of threads to be used
#--ncomponents number of components to be extract by umap algorithm
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
		...
		Rscript "$SCRIPT"/SeqDex/Clustering.R --modelOutput ../RFoutput/2outputRF.txt --gcCovKmersTable ../Coverage/gckCovTable.txt --rRNA16S ../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../Taxonomy/2taxonomyIteration.txt  --network ../Coverage/contigNet.txt --input Kmers --minContigLen 1000 --targetName "$TRG" --threads "$THREADS" --ncomponents 2 --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
#--verbose set to true to print on the terminal performance statistics and percentage of error committed by SVM model 
	Rscript "$SCRIPT"/SeqDex/SVM.R --gcCovKmersTable ../../Coverage/gckCovTable.txt --rRNA16S ../../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../../Taxonomy/1taxonomyIteration.txt,../../Taxonomy/2taxonomyIteration.txt --network ../../Coverage/contigNet.txt --cross 5 --cost 1e-1,1e3 --gamma 1e-5,1e-1 --scale F --minContigLen 1000 --targetName Bacteria,Alphaproteobacteria --TaxaName superkingdom,class --threads "$THREADS" --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2 --verbose T
	cd ..
	if [ $CLUSTER == "yes" ]; then
		echo "umap e dbscan on SVM"
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
		...
		seqtk subseq ../../"${2}".fasta OutputClustering.txt > OutputClusteringSVM_W.fasta
		cd ..
	else
		cd SVMoutput
		cut -f1 1outputSVM.txt > 1outputSVM_names.txt
		seqtk subseq ../"${2}".fasta 1outputDVM_names.txt > 1outputSVM_contigs.fasta
		cd ..
	fi
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
#--verbose set to true to print on the terminal performance statistics and percentage of error committed by RF model 
	Rscript "$SCRIPT"/SeqDex/RFTrans.R --gcCovKmersTable ../../Coverage/gckCovTable.txt --rRNA16S ../../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../../Taxonomy/1taxonomyIteration.txt,../../Taxonomy/2taxonomyIteration.txt --network ../../Coverage/contigNet.txt --minContigLen 1000 --targetName Bacteria,Alphaproteobacteria --TaxaName superkingdom,class --replace T --threads "$THREADS" --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2 --verbose T
	cd ..
	if [ $CLUSTER == "yes" ]; then
...
#--mixedComponents maximum proportion of alternative taxonomy over the total to consider the alternative as erroneous and correct/uniform the CC taxonomy
		...
		Rscript "$SCRIPT"/SeqDex/ClusteringTrans.R --modelOutput ../RFoutput/2outputRF.txt --gcCovKmersTable ../../Coverage/gckCovTable.txt --rRNA16S ../../Taxonomy/rRNA16sTaxonomy2.txt --taxonomy ../../Taxonomy/2taxonomyIteration.txt  --network ../../Coverage/contigNet.txt --input Kmers --minContigLen 1000 --targetName "$TRG" --threads "$THREADS" --ncomponents 2 --Edges 10 --VerticesDegree 5 --componentSize 2 --mixedComponents 0.2
 
		seqtk subseq ../../"${2}".fasta OutputClustering.txt > OutputClusteringRF_W.fasta
		cd ..
		else
		cd RFoutput
		cut -f1 1outputRF.txt > 1outputRF_names.txt
