...
option_list <- list(
  ...
  make_option(c("-L", "--taxaLevels"), action = "store", type = "character", default = "1",
...
if (grepl(opt$taxa, pattern = ",")) {
  TAXA <- as.numeric(unlist(strsplit(opt$taxa, split = ",", fixed = T)))
} else {
    TAXA <- as.numeric(opt$taxa)
}

if (grepl(opt$taxaLevels, pattern = ",")) {
  TAXAlevel <- as.numeric(unlist(strsplit(opt$taxaLevels, split = ",", fixed = T)))
} else {
  TAXAlevel <- as.numeric(opt$taxaLevels)
}
...
suppressWarnings(BlastAll <-BlastTaxaAll(table=BlastFilt,  taxa=TAXA, 
...
if (!is.na(opt$diamond)) {
  ...
  suppressWarnings(DiamondBlastp <-BlastTaxaAll(table=DiamondTaxa, taxa=TAXA, 
                                                blast_input = "chromosome" 
                                                ))
...
for (i in 1:length(TAXAlevel)) {
  if (is.na(opt$diamond)) {
    taxa.level= which(colnames(BlastAll)==taxonomy.df[TAXAlevel[i],1])
    ...
    write.table(mergedNewTax, paste(i,"taxonomyIteration.txt", sep = ""), sep = "\t", quote = F, row.names = F)
  } else {
    taxa.level= which(colnames(BlastAll)==taxonomy.df[TAXAlevel[i],1])
    BlastTD <- TaxonomyDensity(BlastNoNaFilterLength, taxa.level)
    BTDallFilt75 <- BlastTD[which(BlastTD$TaxonDensity>=opt$TaxonDensity),]
    taxa.levelD= which(colnames(DiamondNoNaFilterLength)==taxonomy.df[TAXAlevel[i],1])
