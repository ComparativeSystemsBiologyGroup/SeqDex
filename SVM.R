...
#taxa.name= "superkingdom"#taxonomic level considered
#target.name = "Bacteria" # target taxonomic category 
...
if (grepl(opt$taxonomy, pattern = ",")) {
  list.filetaxa <- as.vector(unlist(strsplit(opt$taxonomy, split = ",", fixed = T)))
} else {
  list.filetaxa <- as.vector(opt$taxonomy)
}
...
#Data------------------------------------------------------------------------
for (j in 1:length(list.filetaxa)) {
  mergedTaxonomy <- read.table(list.filetaxa[j], sep = "\t", stringsAsFactors = F, header = T)
  ...
  cat("percentage of error committed in reclassification, considering number of sequences", 
      file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  suppressWarnings(write.table(error.predN, paste(j,"output_statsSVM.txt", sep = ""), 
  ...
  cat("percentage of error committed in reclassification, considering length of the sequences",  
      file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  suppressWarnings(write.table(error.predL, paste(j,"output_statsSVM.txt", sep = ""), 
                               append = T, sep = "\t", quote = F, row.names = T ))
  cat("\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("performance statistics of SVM model calculated on number of target sequences correctly reclassified",
      file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  suppressWarnings(write.table(statisticsN, paste(j,"output_statsSVM.txt", sep = ""), 
  ...
  cat("performance statistics of SVM model calculated on cumulative amount of target bp correctly reclassified", 
      file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  suppressWarnings(write.table(statisticsL, paste(j,"output_statsSVM.txt", sep = ""), 
                               append = T, sep = "\t", quote = F, row.names = F ))
  ...
  write.table(output, paste(j,"outputSVM.txt", sep = ""), sep = "\t",quote = F, row.names = F)
  ...
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  cat("Homogenety index for SVM machine learning predictions. \n\nThis value have been calculated by dividing number of non zero homogeneous components to number of non zero heterogeneous components", 
      file = paste(j,"output_statsSVM.txt", sep = ""), append = T )
  cat("\n\n", file = paste(j,"output_statsSVM.txt", sep = ""), append = T)
  suppressWarnings(write(h.index, 
                         file = paste(j,"output_statsSVM.txt", sep = ""), 
                               append = T))
  
  save.image(paste(j,"svmModel.RData", sep = ""))
