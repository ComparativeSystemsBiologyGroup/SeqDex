...
modTable <- function(taxa_16S){
  ...
  taxaB <- taxa_16S[which(taxa_16S$V4!="rootrank"),]
  taxaB <- taxaB[,-c(3,5,7,9,11,13,14)]
  colnames(taxaB) <- c("subject",	"superkingdom",	"phylum",	"class",	"order",	"family",	"genus")
  taxa16s <- rbind(taxaA, taxaB)
  return(taxa16s)
}
...
rRNA16sTaxonomy <- rRNA16sTaxonomy[which(rRNA16sTaxonomy$ali_Length>500),]
